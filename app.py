...
数据源切换 (环境变量, 无需改代码):
    DATASOURCE = realtime  (默认, 腾讯+AkShare, 无需 token, 推荐)
    DATASOURCE = tushare   (TuShare, 需 TUSHARE_TOKEN, 有限流)
    DATASOURCE = mock       (强制模拟, 离线调试)
    USE_MANUAL_POOL = true  (慢池用 manual_pool.csv, 通达信132只; false=全市场主板)
"""
...
# 真实/模拟数据源（按需导入；任一缺失会自动降级 MOCK）
try:
    from data_source import DataSource  # TuShare 路径
except Exception:
    DataSource = None

try:
    from realtime_source import RealtimeDataSource  # 腾讯+AkShare 路径
except Exception:
    RealtimeDataSource = None

# 兜底：若 realtime_source 未提供，内置一个始终返回 MOCK 的桩，保证界面永可运行
if RealtimeDataSource is None:
    class _MockRealtime:
        source = "MOCK"
        error = "realtime_source.py 未找到"

        def __init__(self, *a, **kw):
            self._mock = kw.get("pool", [])
    ...
    RealtimeDataSource = _MockRealtime  # type: ignore
...
class MarketSimulator:
    """模拟实时行情: 每轮在 base_price 附近随机游走, 金额随时间放大"""
    ...
    def snapshot(self) -> List[Dict]:
        ...
        return [compute_main_positions(r) for r in rows]
...
def build_data_source():
    """按环境变量选择数据源; 任一环节失败都回退 MOCK, 界面永可运行"""
    mode = os.getenv("DATASOURCE", "realtime").lower()
    use_manual = os.getenv("USE_MANUAL_POOL", "true").lower() in ("1", "true", "yes")

    mock = make_mock_pool()

    if mode == "tushare" and DataSource is not None:
        return DataSource(pool=mock, use_real=True)

    if mode == "mock":
        if DataSource is not None:
            return DataSource(pool=mock, use_real=False)
        return _FallbackDataSource(mock)

    # 默认: realtime (腾讯 + AkShare)
...
class _FallbackDataSource:
    """内置 MOCK 兜底 (data_source.py 缺失时使用)"""
    source = "MOCK"
    error = ""
    ...
    def snapshot(self, pool):
        sim = MarketSimulator(self._pool)
        return sim.snapshot()
...
DEFAULT_PARAMS = {
    ...
    "pe_static_max": 500.0, "pe_ttm_max": 500.0, "pe_dyn_max": 500.0,
    "pe_dyn_static_ratio_max": 1.0, "pe_dyn_ttm_ratio_max": 1.0,
    "peg_max": 1.0, "roe_min": 0.0, "debt_ratio_max": 80.0, "div_yield_min": 0.0,
...
thead th {
    ...
    white-space: nowrap; padding: 2px 4px !important;
    border-bottom: 1px solid #333; color: #bbb;
}
...
def main():
    ...
    header = st.container()
    with header:
        # ① 先取数：pool / snapshot 必须先就绪
        ds = st.session_state.ds
        pool = make_mock_pool()   # 兜底默认，保证下面 len(pool) 不报错
        snapshot = None
        try:
        ...
        if not snapshot:
            snapshot = MarketSimulator(make_mock_pool()).snapshot()
        ...
        # 数据来源标签（真实则显示来源，否则显示降级原因）
        src = getattr(ds, "source", "MOCK")
        err = getattr(ds, "error", "") or ""
        if src in ("Tushare", "Realtime"):
            src_label = src
        else:
            src_label = f"MOCK 模拟" + (f" · {err}" if err else "")
        c5.write(
            f"**最后更新**: {datetime.now().strftime('%H:%M:%S')}  ·  "
            f"下一轮 {refresh_rate}s  ·  数据: {src_label}"
        )
    ...
    for code in exited:
        r = prev_map.get(code, {})
        st.session_state.history.insert(0, (datetime.now().strftime("%H:%M:%S"), "❌ 出局", f"{code} {r.get('name', '')}"))
    ...
    if selected:
        ...
        for col in num_color_cols:
            if col in styled.columns:
                styler = styler.map(
                    lambda v: "color:#ff4d4f" if (isinstance(v, (int, float)) and v > 0)
        ...
        fmt_dict = {c["key"]: f"{{:.{c['decimals']}f}}" for c in COLUMNS
                    if c["decimals"] is not None and c["key"] in cols}
        if fmt_dict:
        ...
        top = sorted(diag.items(), key=lambda x: -x[1])[:5]
        if top:
            st.info("💡 **建议放宽以下过严条件:**")
            for k, v in top:
    ...
    with col_help:
        ...
        - **架构**: 季度慢池(财报) + 盘中快筛(行情) + diff 自动进出
        - **周期股**: 申万行业分级 + 二级精细排除
        - **数据源**: 默认腾讯+AkShare 实时 (无需 token); 设 `DATASOURCE=mock` 强制模拟
        - **慢池**: 放 `manual_pool.csv`(通达信导出) 并设 `USE_MANUAL_POOL=true` 即可启用
        - **字段**: 小数统一2位; 金额按亿
        """)
    ...
    st.caption("💡 启用自动刷新: 安装 `streamlit-autorefresh`, 当前手动刷新请按 F5 / Cmd+R")
